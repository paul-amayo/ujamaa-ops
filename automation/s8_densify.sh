...
echo "n" | MAX_JOBS=4 HIGH_EMBEDDER_CKPT=$EMB \
  ...
  pixi run ns-train high \
    --data $CB --output-dir $CB/splat_runs_S8 --experiment-name fruit_s8_densify \
    ...
    --eval-mode interval --eval-interval 10 || { echo "S8-FAIL"; exit 1; }
echo "S8-TRAINED"
CFG=$(ls -t $CB/splat_runs_S8/fruit_s8_densify/high/*/config.yml | head -1)
...
HIGH_EMBEDDER_CKPT=$EMB pixi run --manifest-path /home/paperspace/code/nerf_new/pixi.toml \
  ...
  | grep -aE "ALIGNED|fruit pixels|tree  pixels|choosing" | sed "s/^/GATES s8 /"
echo "S8-ALL-DONE"
