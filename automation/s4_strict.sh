...
echo "n" | \
  ...
  pixi run ns-train high \
    --data $CB --output-dir $CB/splat_runs_S4 --experiment-name fruit_s4_strict \
    ...
    --pipeline.datamanager.semantic-dir $CB/semantic_v2_S \
    --max-num-iterations 20001 --steps-per-save 9998 \
    --vis tensorboard nerfstudio-data || exit 1
echo "S4-TRAINED"
S4CFG=$(ls -t $CB/splat_runs_S4/fruit_s4_strict/high/*/config.yml | head -1)
sed "s|CFG = Path('.*')|CFG = Path('$S4CFG')|" /home/paperspace/logs/s2_gates_template.py > /home/paperspace/logs/s4_gates.py
cd /home/paperspace/code/aru_sil_core
HIGH_EMBEDDER_CKPT=/home/paperspace/data/high/nerf/04_13D_v2F5/ckpts/model_best.pth \
pixi run --manifest-path /home/paperspace/code/nerf_new/pixi.toml \
  python /home/paperspace/logs/s4_gates.py 2>&1 | grep -aE "RENDERED|fruit pixels|tree  pixels|CROSS-LEVEL|choosing"
echo "S4-ALL-DONE"
