...
echo "n" | \
  ...
  pixi run ns-train high \
    --data $CB --output-dir $CB/splat_runs_S5 --experiment-name fruit_s5_idmap \
    ...
    --pipeline.datamanager.semantic-dir $CB/supervision/strict_fruit_v2 \
    --max-num-iterations 20001 --steps-per-save 9998 \
    --vis tensorboard nerfstudio-data || exit 1
echo "S5-TRAINED"
S5CFG=$(ls -t $CB/splat_runs_S5/fruit_s5_idmap/high/*/config.yml | head -1)
sed "s|CFG = Path('.*')|CFG = Path('$S5CFG')|" /home/paperspace/logs/s2_gates_template.py > /home/paperspace/logs/s5_gates.py
cd /home/paperspace/code/aru_sil_core
HIGH_EMBEDDER_CKPT=/home/paperspace/data/high/nerf/04_13D_v2F5/ckpts/model_best.pth \
pixi run --manifest-path /home/paperspace/code/nerf_new/pixi.toml \
  python /home/paperspace/logs/s5_gates.py 2>&1 | grep -aE "RENDERED|fruit pixels|tree  pixels|CROSS-LEVEL|choosing"
echo "S5-ALL-DONE"
