...
echo "n" | MAX_JOBS=4 HIGH_EMBEDDER_CKPT=$EMB \
  ...
  pixi run ns-train high \
    --data $CB --output-dir $CB/splat_runs_S9 --experiment-name fruit_s9_cascade \
    ...
    --eval-mode interval --eval-interval 10 || { echo "S9-FAIL"; exit 1; }
echo "S9-TRAINED"
CFG=$(ls -t $CB/splat_runs_S9/fruit_s9_cascade/high/*/config.yml | head -1)
...
HIGH_EMBEDDER_CKPT=$EMB pixi run --manifest-path /home/paperspace/code/nerf_new/pixi.toml \
  ...
  | grep -aE "ALIGNED|fruit pixels|tree  pixels|choosing" | sed "s/^/GATES s9 /"
echo "S9-ALL-DONE"
