...
for tau in a.taus:
    ...
    for name in test:
        ...
        nosky = None
        if SKY and (SKY / name).exists(): nosky = torch.from_numpy(np.asarray(Image.open(SKY / name).convert("L")) == 0).cuda()
        if not rows: print(f"[eval] before first render: free {torch.cuda.mem_get_info()[0]/2**30:.2f} GiB, torch reserved {torch.cuda.memory_reserved()/2**30:.2f} GiB; sky masks: {bool(SKY)}", flush=True)
        try:
        ...
        r = {"name": name, "tau": tau, "chunk": cname, "psnr": psnr(im, gt), "psnr_fg": psnr(im, gt, mask) if mask is not None else None,
             "psnr_nosky": psnr(im, gt, nosky) if nosky is not None else None, "nodes": n}
