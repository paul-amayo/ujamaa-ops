...
seen=""; lastp=0; lasterr=$(tr '\r' '\n' < $LOG | grep -caE "Traceback|CUDA out of memory|Error executing|RuntimeError")
while true; do
  ...
  awk '/RESUME/{b=""} {b=b $0 "\n"} END{printf "%s", b}' /home/paperspace/logs/h3dgs_train.log 2>/dev/null | grep -q "H3DGS TRAIN CHAIN DONE\|NO MERGED HIERARCHY\|NO CHUNK COMPLETED" && { echo "[$(date '+%H:%M')] chain finished"; exit 0; }
  pgrep -f "h3dgs_(train_05|resume).sh" > /dev/null || { echo "[$(date '+%H:%M')] trainer script is no longer running"; exit 1; }
  sleep 60
done
