#!/bin/bash
# Klapmuts row-block pilot, stages C-E (block_004). Durable version of the
# /tmp runner lost 2026-07-30 night. Prereqs already on disk: collision-free
# repaint (filtered_semantic_v2 21:30), klapmuts_v1 embedder, row blocks.
set -x
...
cd $ARU
echo "=== STAGE C: semantic PNGs ==="
[ -f "$BD/semantic_v2_B/palette.json" ] || \
pixi run --manifest-path $NS_PIXI python src/scripts/save_filtered_semantic_pngs.py \
  --block-dir "$BD" \
  --semantic-monolithic "$ROOT/filtered_semantic_v2.monolithic" \
  --marker-monolithic "$ROOT/scene_graph/markers_v2.monolithic" || exit 1
