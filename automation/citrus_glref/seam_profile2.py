...
SEAMS = ["21->22", "35->36", "7->8"]; SPAN = 20; W, H = 640, 360
def psnr(a, b):
...
async def main():
    ...
    async with websockets.connect("ws://127.0.0.1:8004/ws", max_size=2**24) as ws:
        for sname in SEAMS:
            ...
            for off in range(-SPAN, SPAN):
                ...
                if sname == "21->22" and off in (-4, -1, 0, 2, 5, 10) and imA and imB: strip[off] = (gt, imA, imB, row)
    json.dump(out, open("/home/paperspace/logs/seam_profile2.json", "w"))
    # strip
    ...
    canvas.save("/home/paperspace/logs/seam_21_22_strip.png")
    # summary
