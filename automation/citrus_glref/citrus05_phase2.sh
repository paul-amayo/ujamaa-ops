#!/bin/bash
# Phase 2 for the demo survey (05_13D): pause the stage-1 fleet, census-seed stage-2 on all 43
...
S=/home/paperspace/data/citrus_all/05_13D_Jackal; B=$S/prod/tassili/blocks_ns/lio_row100
EMB=$S/prod/bateleur/embedder/05_13D_v1g/ckpts/model_best.pth
HJ=$S/prod/bateleur/scene_graph/marker_hierarchy.json
LOG=/home/paperspace/logs/citrus05_phase2.log; TSV=/home/paperspace/logs/citrus05_phase2.tsv
...
say "PHASE-2 START: 43 blocks, census seed on stage1_bg00_glref"
ok=0; bad=0
for BD in $B/block_[0-9][0-9][0-9]; do
  ...
  CENSUS_EMBEDDER=$EMB CENSUS_HIERARCHY=$HJ bash /home/paperspace/logs/censusinit_block_glref.sh $BD > /home/paperspace/logs/c05p2_$N.log 2>&1
  rc=$?; w=$(( $(date +%s)-t0 ))
  if [ $rc -eq 0 ]; then ok=$((ok+1)); echo -e "$N\t$w\tok" >> $TSV; say "$N seeded in ${w}s ($(grep -c REPL-INIT0 /home/paperspace/logs/c05p2_$N.log) init)"
  else bad=$((bad+1)); echo -e "$N\t$w\tFAIL" >> $TSV; say "$N FAILED rc=$rc: $(grep -aE 'REPL-FAIL|Error' /home/paperspace/logs/c05p2_$N.log | tail -1 | cut -c1-120)"; fi
done
...
if [ $ok -ge 40 ]; then
  ...
  say "too many failures ($bad) — demo NOT repointed; legacy render service left as is"
fi
