#!/bin/bash
# 04 EXTRAPOLATION CHAIN (2026-08-02) — per block: image-mode scored fruit
# ledger + hierarchy-projection tree id maps -> compile -> train -> gates.
#   Order: block_001 FIRST (s6_b001 = the s5 coverage fix: image-mode fruit
#   on every frame vs the 25-mask collapse), then 000 002 003 004 005.
#   Per block: F fruit ledger (SAM3 IMAGE mode — Paul's rule, never video)
#              K sky+fg masks | T tree id maps (r6 projection, id-native)
#              C compile (idmap_dir + strict fruit + manifest)
#              D lidar init + depth | E train s6 recipe + gates
# 05: blocked on kf20cm pose dump — prints TODO, no silent skip.
set -x
ARU=/home/paperspace/code/aru_sil_core
NS_PIXI=/home/paperspace/code/nerf_new/pixi.toml
SAM3_PIXI=/home/paperspace/code/sam3/pixi.toml
IS_PIXI=$ARU/src/thirdparty/InstantSplat/pixi.toml
PYNS=/home/paperspace/code/nerf_new/.pixi/envs/default/bin/python
SCR=$ARU/src/scripts
LOG=/home/paperspace/logs
D=/home/paperspace/data/citrus_all/04_13D_Jackal
ROOTB=$D/blocks_ns/lio_row6F
EMB=/home/paperspace/data/high/nerf/04_13D_v2F5/ckpts/model_best.pth

while ps -eo cmd | grep -E "ns-train" | grep -v grep > /dev/null; do sleep 60; done
echo "=== CHAIN-04 START $(date) ==="

for N in 001 000 002 003 004 005; do
  BD=$ROOTB/block_$N
  echo "=== BLOCK $N ==="
  ( set -e
    cd $ARU
    echo "--- F: fruit image ledger ---"
    [ -f "$D/sam3_fruit_img_b$N/clip_000/frame_entries.json" ] || \
      pixi run --manifest-path $SAM3_PIXI python $SCR/fruit_image_ledger.py \
        --data-dir $D --block-dir $BD --out-name sam3_fruit_img_b$N
    echo "--- K: sky + fg masks ---"
    $PYNS - "$BD" <<'PY'
import json, sys
from pathlib import Path
B = Path(sys.argv[1])
tj = "transforms.json.orig" if (B/"transforms.json.orig").exists() else "transforms.json"
names = [Path(f["file_path"]).name for f in json.loads((B/tj).read_text())["frames"]]
Path("$D/chain04_frames.json").write_text(json.dumps(sorted(names)))
print(f"{len(names)} frames")
PY
    pixi run --manifest-path $SAM3_PIXI python $SCR/build_sky_masks.py \
      --data-dir $D --keep-frames $D/chain04_frames.json
    python3 $SCR/build_fg_masks.py --data-dir $D --keep-frames $D/chain04_frames.json
    echo "--- T: tree id maps (hierarchy projection) ---"
    [ -f "$BD/supervision_trees_r6/meta.json" ] || \
      pixi run --manifest-path $NS_PIXI python $SCR/r6_project_idmaps.py \
        --data-dir $D --block-dir $BD
    echo "--- C: compile ---"
    python3 $SCR/compile_supervision.py --block-dir $BD \
      --tree-source idmap_dir --tree-src-dir $BD/supervision_trees_r6 \
      --hierarchy $D/scene_graph_v4/marker_hierarchy_fruit5.json \
      --fruit-ledger-glob "$D/sam3_fruit_img_b$N/clip_*/frame_entries.json" \
      --out-dir $BD/supervision/strict_img_v1
    $PYNS - "$BD" "$D" <<'PY'
import json, sys
from pathlib import Path
B, ROOT = Path(sys.argv[1]), Path(sys.argv[2])
d = json.loads((B/"transforms.json").read_text())
n = 0
for f in d["frames"]:
    mp = ROOT/"fg_masks"/Path(f["file_path"]).name
    if mp.exists():
        f["mask_path"] = str(mp); n += 1
(B/"transforms.json").write_text(json.dumps(d, indent=2))
print(f"[{B.name}] mask_path on {n}/{len(d['frames'])}")
PY
    echo "--- D: lidar init + depth ---"
    [ -f "$BD/init_lidar.ply" ] || [ -f "$BD/init_da3.ply" ] || \
      pixi run --manifest-path $IS_PIXI python $SCR/lidar_init_per_block.py \
        --block-dir $BD --root $D --pad-x 7.5 --cross-row-median
    [ -f "$BD/lidar_depth_morph.npz" ] || \
      pixi run --manifest-path $NS_PIXI python $SCR/build_block_lidar_depth.py \
        --block-dir $BD --root $D
    echo "--- E: train s6_b$N ---"
    cd /home/paperspace/code/nerf_new
    echo "n" | MAX_JOBS=4 HIGH_EMBEDDER_CKPT=$EMB \
      LIDAR_DEPTH_NPZ=$BD/lidar_depth_morph.npz \
      pixi run ns-train high \
        --data $BD --output-dir $BD/splat_runs_S6 --experiment-name fruit_s6_b$N \
        --pipeline.model.rasterize-mode antialiased \
        --pipeline.model.output-depth-during-training True \
        --pipeline.model.high-loss-weight 1.0 \
        --pipeline.model.high-loss-fruit-weight 10.0 \
        --pipeline.model.fruit-protect True \
        --pipeline.model.sky-loss-lambda 1.0 \
        --pipeline.model.report-masked-metrics True \
        --pipeline.datamanager.semantic-dir $BD/supervision/strict_img_v1 \
        --max-num-iterations 20001 --steps-per-save 9998 \
        --vis tensorboard nerfstudio-data \
        --eval-mode interval --eval-interval 10
    CFG=$(ls -t $BD/splat_runs_S6/fruit_s6_b$N/high/*/config.yml | head -1)
    sed "s|CFG = Path('.*')|CFG = Path('$CFG')|" $LOG/s2_gates_template.py > $LOG/s6_gates_b$N.py
    cd $ARU && HIGH_EMBEDDER_CKPT=$EMB \
      pixi run --manifest-path $NS_PIXI python $LOG/s6_gates_b$N.py 2>&1 \
      | grep -aE "RENDERED|fruit pixels|tree  pixels|CROSS-LEVEL|choosing" \
      | sed "s/^/GATES b$N /"
    echo "BLOCK-DONE $N"
  ) || echo "BLOCK-FAIL $N"
done
echo "TODO-05: kf20cm pose dump missing for 05_13D_Jackal — blocks/hierarchy/"
echo "  fruit chain for 05 needs it first (then: recluster_05 exists, fruit"
echo "  image ledgers per block, same compile+train)."
echo "=== CHAIN-04-ALL-DONE $(date) ==="
