#!/bin/bash
# PROD IMAGE-ONLY RECIPE — bare frames -> HiGH splat + hierarchy + manifests.
# Codified from the sukuma corrections (2026-09-02/03) and ratified by the
# gendia fleet rebuild (2026-09-03). Load-bearing choices, each measured:
#   * working frames at WIDTH 1080 (portrait 1080x1920) — long-edge-1080 was
#     half the -6 dB ingestion drift
#   * OPENCV camera model + EXHAUSTIVE matching — sequential-only never binds
#     a down-and-back walk's two passes (the other half of the -6 dB)
#   * init = COLMAP SPARSE CLOUD (init.ply). The DA3 fused cloud loses to it
#     by 3.2 dB train on identical data (sukuma arm A, 23.27 vs 20.11): its
#     far-field range smear seeds misplaced mass that stage-1 fights instead
#     of prunes. v1's inverted result came from a junk sequential SfM cloud.
#   * DA3 keeps TWO jobs: metric scale (pose-free pass) and per-frame depth
#     for the SAM3 mask lift — splat-rendered depth reclustering measured
#     WORSE (22 towers/looser bboxes vs 28/tighter, 2026-09-03).
#   * sky-loss only when the capture actually sees sky (--sky 1).
#
# usage: prod_image_recipe.sh <survey_dir> [--prompt tree] [--eps 0.3]
#        [--det-dist 30] [--sky 0] [--iters 5000] [--structure-only]
# expects <survey_dir>/images_orig/ (any *.png/*.jpg) or a prepared
# <survey_dir>/images/ (image_<0..N-1>.png working set).
set -uo pipefail
SURVEY=$(realpath "${1:?usage: prod_image_recipe.sh <survey_dir> [--prompt X ...]}")
shift
PROMPT="tree"; EPS=0.3; DETDIST=30; SKY=0; ITERS=5000; STRUCTURE_ONLY=0
...
say "working set: $N frames @ ${IW}x${IH}"
...
# markers may write to prod homes or the survey root depending on layout age
...
# ---- 8. HiGH train (features ON) + export + manifests ---------------------
SKYFLAGS=""
...
( cd "$NS" && echo n | MAX_JOBS=4 PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
  ...
  > "$LOGS/recipe_${NAME}_train.log" 2>&1 || gate "HiGH train"
...
( cd "$NS" && HIGH_EMBEDDER_CKPT=$EMB env -u LD_LIBRARY_PATH -u LD_PRELOAD \
