...
# CRITICAL: uses lidar_depth_zbuf_trainaligned.npz, NOT lidar_depth_morph.npz.
...
T0=$(date +%s)
echo "n" | MAX_JOBS=4 \
  LIDAR_DEPTH_NPZ=$BD/lidar_depth_zbuf_trainaligned.npz \
  LIDAR_DEPTH_LAMBDA=0.5 \
  pixi run ns-train high \
    --data $BD --output-dir $BD/splat_runs_BASE --experiment-name base_L095_sky_zbuf \
    ...
    --eval-mode interval --eval-interval 10 || { echo "ZBUF-FAIL"; exit 1; }
echo "BASE-DONE base_L095_sky_zbuf in $(( ($(date +%s)-T0)/60 )) min"
echo "=== L095-ZBUF-ALL-DONE $(date) ==="
