#!/bin/bash
# G0 25% eval orchestrator: wait for checkpoint-2400, pause training, run the
# paired eval, resume training. Detached-safe; every step logged.
set -u
RUN=/home/paperspace/data/g0_run
LOG=~/logs/g0_qlora.log
log() { echo "$(date -Is) [eval50] $*" >> "$LOG"; }

log "waiting for checkpoint-2400"
until [ -d "$RUN/checkpoint-2400" ]; do sleep 120; done
...
log "running paired eval on checkpoint-2400"
~/envs/hfeval/bin/python /home/paperspace/code/automation/g0_qlora/eval_ckpt.py \
  --ckpt "$RUN/checkpoint-2400" --out "$RUN/eval50.json" >> "$LOG" 2>&1 \
