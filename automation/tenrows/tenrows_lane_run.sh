#!/bin/bash
# tenrows_lane_run.sh <lane dir> [ITERS=60000] [DENSIFY_UNTIL=45000] [GRAD=0.0075] [CAP=8000000] — one ten_rows lane, full
# 15 Hz stream, overtrained (Paul, 2026-09-26): SfM on all frames (GPU SIFT + exhaustive + GLOMAP, fixed ZED-conf camera),
# warped onto the LiDAR odometry (metric; a 45 m lane SfM bends ~1.5 m at the ends, so Sim(3) + LOESS field + windowed
# yaw, LANE_WARP=0 for the rigid hybrid Sim(3)), LiDAR init from the raw scans at every frame, H3DGS train_single with a
# long densifying schedule under a gaussian budget (CAP; without one the grad 0.0075 schedule grew 1 M gaussians per 750
# iterations and would have needed ~50 M by 45k on a 40 GB GPU), no depth prior, no scaffold, hierarchy, train_post 30k,
# then the per-chunk evaluator on the lane's own every-10th held-out frames + a training-view sample + renders.
set -u
LD=${1:?lane dir}; ITERS=${2:-60000}; DU=${3:-45000}; GRAD=${4:-0.0075}; CAP=${5:-8000000}
...
say "=== lane $(basename $LD): $(ls $LD/images | wc -l) frames; schedule iters $ITERS densify-until $DU grad $GRAD budget $CAP gaussians"
# 1. SfM on all frames
...
# 2. LO base + placement on the LO (warp by default) + LiDAR init
