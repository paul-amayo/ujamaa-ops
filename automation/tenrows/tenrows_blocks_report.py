...
for bd in sorted(B.glob("block_0??")):
    ...
    evr = bd.parent / f"{n}_ref" / "stage1_eval.json"; mr = json.load(open(evr))["results"] if evr.exists() else {}
    rows.append((n, ntr, float(r.get("wall_s", "nan")), float(r.get("train_psnr_last1k", "nan")), m.get("psnr", np.nan), m.get("ssim", np.nan), m.get("lpips", np.nan), mr.get("psnr", np.nan), mr.get("ssim", np.nan), mr.get("lpips", np.nan)))
print(f"{'block':10s} {'kf':>4s} {'wall_s':>7s} {'trainPSNR':>9s} {'evalPSNR':>8s} {'SSIM':>7s} {'LPIPS':>7s} | {'REF PSNR':>8s} {'SSIM':>7s} {'LPIPS':>7s}")
for r in rows: print(f"{r[0]:10s} {r[1]:4d} {r[2]:7.0f} {r[3]:9.2f} {r[4]:8.2f} {r[5]:7.4f} {r[6]:7.4f} | {r[7]:8.2f} {r[8]:7.4f} {r[9]:7.4f}")
ev = np.array([r[4] for r in rows]); evr_ = np.array([r[7] for r in rows]); wall = np.array([r[2] for r in rows]); kf = np.array([r[1] for r in rows])
ok = np.isfinite(ev)
...
if np.isfinite(evr_).any(): print(f"[blocks] REFINED-pose pass: n={np.isfinite(evr_).sum()} eval PSNR median {np.nanmedian(evr_):.2f} (min {np.nanmin(evr_):.2f}, max {np.nanmax(evr_):.2f}); median gain over odometry poses {np.nanmedian(evr_ - ev):+.2f} dB")
fig, ax = plt.subplots(1, 2, figsize=(16, 5.5), dpi=120)
x = np.arange(len(rows)); ax[0].bar(x - 0.2, ev, width=0.4, color="#4a8", label="odometry poses (roll-corrected)"); ax[0].bar(x + 0.2, evr_, width=0.4, color="#c60", label="COLMAP-refined poses"); ax[0].legend(fontsize=8)
ax[0].set_xticks(x); ax[0].set_xticklabels([r[0][-3:] for r in rows], rotation=90)
for i, r in enumerate(rows): ax[0].text(i, (np.nanmax([ev[i], evr_[i]]) if np.isfinite(ev[i]) else 0) + 0.2, f"{r[1]}", ha="center", fontsize=7)
